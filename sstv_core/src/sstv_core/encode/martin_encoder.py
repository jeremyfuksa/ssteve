...
class MartinM1Encoder:
    ...
    @property
    def config(self) -> MartinM1EncoderConfig:
        return self._config
    ...
    def encode_scanline(self, rgb_row: np.ndarray, line_number: int) -> np.ndarray:
        ...
        Martin M1 line structure: sync + sep + green + sep + blue + sep + red
        """
        cfg = self._config
        audio_parts = []
    ...
    def get_progress(self) -> EncoderProgress:
        ...
        line_duration_sec = (cfg.samples_per_sync + cfg.samples_per_separator * 3 +
                            cfg.samples_per_color_line * 3) / cfg.sample_rate
    ...
    def get_total_duration_sec(self) -> float:
        ...
        cfg = self._config
        line_samples = (cfg.samples_per_sync + cfg.samples_per_separator * 3 +
                       cfg.samples_per_color_line * 3)
